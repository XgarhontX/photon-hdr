...
// C[2] = blue.xy
// C[3] = white.xy
vec3x3 RGBtoXYZ_f33(float4x2 C, float Y) {
  float2 red = C[0];
  float2 green = C[1];
  float2 blue = C[2];
  float2 white = C[3];
  ...
  vec3x3 M;
  M[0] = Sr * vec3(red.x, red.y, 1.0 - red.x - red.y);
  M[1] = Sg * vec3(green.x, green.y, 1.0 - green.x - green.y);
  M[2] = Sb * vec3(blue.x, blue.y, 1.0 - blue.x - blue.y);

  return M;
}

vec3x3 XYZtoRGB_f33(float4x2 C,
                      float Y) {
...
vec3x3 scale_matrix_diagonal_f33_f3(vec3x3 A,
                                      vec3 v) {
  vec3x3 B = A;
  B[0][0] = A[0][0] * v[0];
  B[1][1] = A[1][1] * v[1];
...
static const vec3x3 AP0_XYZ_TO_RGB = XYZtoRGB_f33(Chromaticities_AP0, 1.0);
static const vec3x3 AP0_RGB_TO_XYZ = RGBtoXYZ_f33(Chromaticities_AP0, 1.0);
...
static const vec3x3 AP1_XYZ_TO_RGB = XYZtoRGB_f33(Chromaticities_AP1, 1.0);
static const vec3x3 AP1_RGB_TO_XYZ = RGBtoXYZ_f33(Chromaticities_AP1, 1.0);

static const vec3x3 AP0_TO_AP1 = mul(AP0_RGB_TO_XYZ, AP1_XYZ_TO_RGB);
static const vec3x3 AP1_TO_AP0 = mul(AP1_RGB_TO_XYZ, AP0_XYZ_TO_RGB);
...
// Wrappers for fixed-size vec3 array return since HLSL does not allow array return types
struct JMhCornerTable {
  vec3 values[8];  // totalCornerCount = 8
};
struct SortedHues {
...
struct CuspTable {
  vec3 values[362];  // totalTableSize = 362
};
struct ReachMTable {
  float values[362];  // totalTableSize = 362
};
...
const float L_A = 100.;
const float Y_b = 20.;
const vec3 surround = vec3(0.9, 0.59, 0.9);  // Dim surround
...
const vec3x3 MATRIX_IDENTITY = vec3x3(
    1, 0, 0,
    0, 1, 0,
    0, 0, 1);
...
struct JMhParams {
  ...
  vec3x3 MATRIX_RGB_to_CAM16_c;
  vec3x3 MATRIX_CAM16_c_to_RGB;
  vec3x3 MATRIX_cone_response_to_Aab;
  vec3x3 MATRIX_Aab_to_cone_response;
...
struct ODTParams {
  ...
  vec3 TABLE_gamut_cusps[362];      // totalTableSize
...
vec3 RGB_to_Aab(vec3 RGB,
                  JMhParams p) {
  vec3 rgb_m = mul(RGB, p.MATRIX_RGB_to_CAM16_c);
  ...
  vec3 Aab = mul(rgb_a, p.MATRIX_cone_response_to_Aab);

  return Aab;
}

vec3 Aab_to_JMh(vec3 Aab,
                  JMhParams p) {
  vec3 JMh = { 0., 0., 0. };
  if (Aab[0] <= 0.) {
    return JMh;
...
vec3 RGB_to_JMh(vec3 RGB,
  ...
  vec3 JMh = Aab_to_JMh(Aab, p);

  return JMh;
}

vec3 JMh_to_Aab(vec3 JMh,
                  JMhParams p) {
  float J = JMh[0];
  float M = JMh[1];
  float h = JMh[2];
...
vec3 Aab_to_RGB(vec3 Aab,
                  JMhParams p) {
  vec3 rgb_a = mul(Aab, p.MATRIX_Aab_to_cone_response);
  ...
  vec3 rgb = mul(rgb_m, p.MATRIX_CAM16_c_to_RGB);
...
vec3 JMh_to_RGB(vec3 JMh,
  ...
  vec3 rgb = Aab_to_RGB(Aab, p);
  return rgb;
}

vec3 clamp_AP0_to_AP1(vec3 aces,
                        float clamp_lower_limit,
                        float clamp_upper_limit) {
  vec3 AP1 = mul(aces, AP0_TO_AP1);
  vec3 AP1_clamped = clamp(AP1, clamp_lower_limit, clamp_upper_limit);
  vec3 AP0_clamped = mul(AP1_clamped, AP1_TO_AP0);
...
float reach_M_from_table(float h,
  ...
  return mix(table[i_lo], table[i_hi], t);
}
...
vec3 chroma_compress_fwd(vec3 JMh,
                           float tonemapped_J,
                           ODTParams p,
                           bool invert = false) {
  float J = JMh[0];
  ...
  vec3 output = vec3(tonemapped_J, M_compr, h);
  return output;
}

vec3 chroma_compress_inv(vec3 JMh,
                           float J,
                           ODTParams p,
                           bool invert = false) {
  ...
  vec3 output = vec3(J, M, h);
  return output;
}

vec3 tonemap_and_compress_fwd(vec3 JMh,
                                ODTParams p) {
  ...
  vec3 JMh_tc = chroma_compress_fwd(JMh, J_ts, p, false);

  return JMh_tc;
}

vec3 tonemap_and_compress_inv(vec3 JMh_tc,
                                ODTParams p) {
  ...
  vec3 JMh = chroma_compress_inv(JMh_tc, J, p, true);
...
float hueDependentUpperHullGamma(float h,
  ...
  return mix(gamma_table[i_lo], gamma_table[i_hi], t);
}
...
vec3 compress_gamut(vec3 JMh,
  ...
  vec3 JMhcompressed = vec3(J_intersect_source + remapped_M * gamut_slope,
                                remapped_M,
                                h);
...
float2 cusp_from_table(float h,
  ...
  float cusp_J = mix(lo[0], hi[0], t);
  float cusp_M = mix(lo[1], hi[1], t);
...
float compute_focus_J(float cusp_J, float mid_J, float limit_J_max) {
  return mix(cusp_J, mid_J, min(1, cusp_mid_blend - (cusp_J / limit_J_max)));
}
...
HueDependentGamutParams init_HueDependentGamutParams(float hue, ODTParams p) {
  ...
  hdp.gamma_top_inv = mix(p.TABLE_upper_hull_gamma[i_hi - 1], p.TABLE_upper_hull_gamma[i_hi], t);
  hdp.focus_J = compute_focus_J(hdp.JMcusp[0], p.mid_J, p.limit_J_max);
  hdp.analytical_threshold = mix(hdp.JMcusp[0], p.limit_J_max, focus_gain_blend);

  return hdp;
}

vec3 gamut_compress_fwd(vec3 JMh,
                          ODTParams p) {
  const float J = JMh[0];
  const float M = JMh[1];
  const float h = JMh[2];
...
vec3 gamut_compress_inv(vec3 JMh,
                          ODTParams p) {
  const float J = JMh[0];
  const float M = JMh[1];
  const float h = JMh[2];
...
bool any_below_zero(vec3 newLimitRGB) {
...
JMhParams init_JMhParams(float4x2 prims) {
  ...
  const vec3x3 MATRIX_16 = XYZtoRGB_f33(CAM16_PRI, 1.0);

  const vec3x3 base_cone_response_to_Aab = vec3x3(
      2., 1., 1. / 9.,
      1., -12. / 11., 1. / 9.,
      1. / 20., 1. / 11., -2. / 9.);

  const vec3x3 RGB_TO_XYZ = RGBtoXYZ_f33(prims, 1.0);
  const vec3 XYZ_w = mul(ref_luminance.xxx, RGB_TO_XYZ);

  float Y_w = XYZ_w[1];

  // Step 0 - Converting CIE XYZ tristimulus values to sharpened RGB values
  vec3 RGB_w = mul(XYZ_w, MATRIX_16);
  ...
  const vec3 D_RGB = vec3(
      F_L_n * Y_w / RGB_w[0],
      F_L_n * Y_w / RGB_w[1],
      F_L_n * Y_w / RGB_w[2]);

  const vec3 RGB_wc = vec3(
      D_RGB[0] * RGB_w[0],
      D_RGB[1] * RGB_w[1],
      D_RGB[2] * RGB_w[2]);

  const vec3 RGB_Aw = vec3(
  ...
  vec3x3 cone_response_to_Aab = mul(cam_nl_scale * MATRIX_IDENTITY, base_cone_response_to_Aab);
  ...
  vec3x3 M1 = mul(RGB_TO_XYZ, MATRIX_16);
  vec3x3 M2 = ref_luminance * MATRIX_IDENTITY;
  vec3x3 MATRIX_RGB_to_CAM16 = mul(M1, M2);
  vec3x3 MATRIX_RGB_to_CAM16_c = mul(MATRIX_RGB_to_CAM16, scale_matrix_diagonal_f33_f3(MATRIX_IDENTITY, D_RGB));

  vec3x3 MATRIX_cone_response_to_Aab = vec3x3(
...
vec3 generate_unit_cube_cusp_corners(int corner) {
  vec3 result;
...
void build_limiting_cusp_corners_tables(out vec3 RGB_corners[8],  // cuspCornerCount = 6
                                        out vec3 JMh_corners[8],  // cuspCornerCount = 6
                                        JMhParams params,
                                        float peakLuminance) {
  ...
  vec3 temp_RGB_corners[6];  // cuspCornerCount = 6
  vec3 temp_JMh_corners[6];  // cuspCornerCount = 6
...
JMhCornerTable find_reach_corners_table(JMhParams params_reach,
  ...
  int min_index = 0;
  for (int i = 0; i != cuspCornerCount; i = i + 1) {
    const vec3 rgb_vector = generate_unit_cube_cusp_corners(i);
    ...
    while ((upper - lower) > reach_cusp_tolerance) {
      ...
      vec3 test_corner = test * rgb_vector;
...
SortedHues extract_sorted_cube_hues(vec3 reach_JMh[8],  // totalCornerCount = 8
                                    vec3 limit_JMh[8]   // totalCornerCount = 8
) {
...
float2 find_display_cusp_for_hue(float hue,
                                 vec3 RGB_corners[8],  // totalCornerCount = 8
                                 vec3 JMh_corners[8],  // totalCornerCount = 8
                                 JMhParams params,
                                 float2 previous) {
  ...
  // search by mixing between RGB corners for the hue
  ...
  while ((upper_t - lower_t) > display_cusp_tolerance) {
    ...
    sample = mix(cusp_lower, cusp_upper, sample_t);
...
CuspTable make_uniform_hue_gamut_table(JMhParams reach_params,
  ...
  vec3 limiting_RGB_corners[8];  // totalCornerCount = 8
  vec3 limiting_JMh_corners[8];  // totalCornerCount = 8
...
ReachMTable make_reach_m_table(JMhParams params,
  ...
  for (int i = 0; i < tableSize; i = i + 1) {
    ...
    while ((outside != true) & (high < search_maximum)) {
      vec3 searchJMh = { limitJmax, high, hue };
      vec3 newLimitRGB = JMh_to_RGB(searchJMh, params);
    ...
    while (high - low > 1e-2) {
      ...
      vec3 searchJMh = { limitJmax, sampleM, hue };
      vec3 newLimitRGB = JMh_to_RGB(searchJMh, params);
      outside = any_below_zero(newLimitRGB);
      if (outside) {
        high = sampleM;
...
bool outside_hull(vec3 rgb, float maxRGBtestVal) {
...
bool evaluate_gamma_fit(float2 JMcusp,
  ...
  for (int testIndex = 0; testIndex < test_count; testIndex = testIndex + 1) {
    ...
    // Store JMh values
    vec3 approximate_JMh = vec3(approxLimit_J, approxLimit_M, test_JMh[testIndex][2]);

    // Convert to RGB
    vec3 newLimitRGB = JMh_to_RGB(approximate_JMh, limit_params);
...
UpperHullGammaTable make_upper_hull_gamma_table(vec3 gamutCuspTable[362],  // totalTableSize
                                                ODTParams p) {
...
vec3 outputTransform_fwd(vec3 aces, ODTParams p) {
  vec3 AP0_clamped = clamp_AP0_to_AP1(aces, 0., p.ts.forward_limit);

  vec3 JMh = RGB_to_JMh(AP0_clamped, p.input_params);

  vec3 tonemappedJMh = tonemap_and_compress_fwd(JMh, p);

  vec3 compressedJMh = gamut_compress_fwd(tonemappedJMh, p);

  vec3 RGBout = JMh_to_RGB(compressedJMh, p.limit_params);

  return RGBout;
}

vec3 outputTransform_inv(vec3 RGBout,
                           ODTParams p) {
  vec3 compressedJMh = RGB_to_JMh(RGBout, p.limit_params);

  vec3 tonemappedJMh = gamut_compress_inv(compressedJMh, p);

  vec3 JMh = tonemap_and_compress_inv(tonemappedJMh, p);

  vec3 aces = JMh_to_RGB(JMh, p.input_params);
